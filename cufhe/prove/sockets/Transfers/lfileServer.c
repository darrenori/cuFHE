...
void send_file(FILE *fp, int sockfd){
  ...
  while(fgets(dataa, SIZEE, fp) != NULL) {
    if (send(sockfd, dataa, sizeof(dataa), 0) == -1) {
...
int main(){
  ...
  int count = fread(&buffer, sizeof(char), 1024, fp2);
  printf("Data read from file: %s \n", buffer);
   
  int n;
  char data[SIZE] = {0};
  while(fgets(data, SIZE, fp2) != NULL) {
  	printf("Hello\n");
