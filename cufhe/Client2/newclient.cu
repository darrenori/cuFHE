...
int main(int argc, char const* argv[])
{
    ...
    Client_socket K;
    K.start_everything(4385, "server");
    K.receive_file();
    K.close_socket();
   ...
   for (int i = 0; i < numBits; i ++) {
    ...
    // SEND THE INPUT AND OPERATOR TO SERVER
    Client_socket C;
    C.start_everything(4387, "client");
    C.transmit_file();
    C.close_socket();

    gettimeofday(&end, NULL);
    get_time = (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec) * 1.0E-6;
    printf("Time taken to send(including wait): %lf[sec]\n", get_time);
